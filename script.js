...
function decimalParaBinarioFracionado() {
  let entrada = document.getElementById("1").value.replace(",", ".");
  let decimal = parseFloat(entrada);
  let binarioInteiro = decimalParaBinario(Math.floor(decimal));
  let resto = decimal - Math.floor(decimal);
  let binarioFracionado = "";

  while (resto !== 0 && binarioFracionado.length < 10) {
    resto *= 2;
    if (resto >= 1) {
      binarioFracionado += "1";
      resto -= 1;
  ...
  let resultadoBinario = binarioInteiro + "." + binarioFracionado;
  let resultadoDecimal = parseInt(binarioInteiro, 2) + parseInt(binarioFracionado, 2) * Math.pow(2, -binarioFracionado.length);

  document.getElementById("resultado").innerHTML = "O resultado em binário é: " + resultadoBinario;
}
